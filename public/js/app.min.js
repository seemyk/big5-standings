...
let currentView = document.getElementById("viewDropdown").value;
...
function renderScorers(scorers) {
  ...
  scorers.forEach((item, index) => {
    ...
    let matches =
      item.playedMatches && item.playedMatches > 0 ? item.playedMatches : 1;
...
function sortTable(columnKey) {
  ...
  allTeamsGlobal.sort((a, b) => {
    ...
    if (columnKey === "points" && a.points === b.points) {
      if (a.goalDifference < b.goalDifference) return -1 * currentSortOrder;
      if (a.goalDifference > b.goalDifference) return 1 * currentSortOrder;
      return 0;
    }
...
function updateAssistsSortIndicators() {
  const ths = document.querySelectorAll("th.sortable");
  ths.forEach((th) => th.classList.remove("sorted-asc", "sorted-desc"));
  const headers = { player: 2, club: 3, league: 4, assists: 5, apm: 6, ga: 7 };
